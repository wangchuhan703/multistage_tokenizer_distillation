...
def main(args):
    ...
    if rank == 0:
        ...
        wandb_logger = wandb.init(project='diff_toknenizer', name=f'exp{experiment_index:03d}-{model_string_name}-jepa{args.jepa_loss_weight}-blocktype{args.block_type}', dir=args.results_dir)
    else:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for data, masks_enc, masks_pred in loader:
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype):  
                dec_loss, codebook_loss, info = vq_model(imgs, masks_enc, masks_pred)
            ...
            if train_steps % args.log_every == 0 or train_steps == max_train_steps:
                ...
                if rank == 0 and wandb_logger is not None:
                    log_dict = {"lr": optimizer.param_groups[0]["lr"], "train_loss": avg_loss, 'jepa_loss': jepa_loss.item(), 'diff_loss': codebook_loss[0].item(), 'lpips_loss': codebook_loss[1].item(), 'repa_loss': codebook_loss[2].item()}
            ...
            if train_steps % args.vis_every == 0:
                ...
                with torch.no_grad(), torch.cuda.amp.autocast(dtype=ptdtype):
                    ...
                    recons_imgs = vq_model.module.decode(quant, num_steps=250, cfg_scale=args.cfg_scale, guidance_high=0.7)
                ...
                if args.val_data_path is not None:
                    ...
                    for x, _ in tqdm(val_loader, desc=f'evaluation for step {train_steps:07d}', disable=not rank == 0):
                        ...
                        with torch.no_grad(), torch.cuda.amp.autocast(dtype=ptdtype):
                            ...
                            recon_x = vq_model.module.decode(quant, num_steps=50, cfg_scale=args.cfg_scale, guidance_high=0.7)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--repa-patch-size', type=int, default=8, help='repa patch size')
