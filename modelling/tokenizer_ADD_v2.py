...
class AEDiffModel(AEModel):
    ...
    def decode_teacher(self, input, s_start_stage, input_student, num_steps=1, cfg_scale=1.0, guidance_low=0., guidance_high=1., clip=False):
        b, _, h, w = input.shape
        quant, diff, info = self.encode(input)
        x_end_teacher_list = list()

        
xT = (1 - start_noise_ratio).view(-1,1,1,1) * xT + start_noise_ratio.view(-1,1,1,1) * noises
        # print("self.multiscale_decoder: ", self.multiscale_decoder)
        if self.multiscale_decoder:
            start_noise_ratio = self.sit_loss.timesteps_per_stage[s_start_stage][0].item()
            start_noise_ratio = start_noise_ratio / 1000.0
            teacher_noise_ratio = (self.sit_loss.timesteps_per_stage[s_start_stage - 1][0].item() - start_noise_ratio) * torch.rand(1).item()
            
            t_start_stage = s_start_stage - 1
            init_factor = 2 ** (self.num_multiscale_stages - t_start_stage - 1)
            image_size = self.config.image_size // init_factor
            xT = input_student
            for d in range(1, self.num_multiscale_stages - t_start_stage):
                xT = F.interpolate(xT, size=(h // (2 ** d), w // (2 ** d)), mode="bilinear")
            noises = torch.randn_like(xT)
            xT = xT + teacher_noise_ratio.view(-1,1,1,1) * noises

            for stage_idx in range(t_start_stage, self.num_multiscale_stages):
                # set the number of inference steps
                timestep_start = self.sit_loss.timesteps_per_stage[stage_idx][0].item()
                timestep_end = self.sit_loss.timesteps_per_stage[stage_idx][-1].item()
                timesteps = np.linspace(timestep_start, timestep_end, num_steps)
        ...
        return x_end_teacher_list, t_start_stage, diff, info
    ...
    def decode_student(self, input, num_steps=1, cfg_scale=1.0, guidance_low=0., guidance_high=1., clip=False):
        ...
        if self.multiscale_decoder:
            ...
            # 3 - T, backward
            # teacher xT - 1
            s_start_stage = torch.randint(1, self.num_multiscale_stages, (1,)).item()
            start_noise_ratio = self.sit_loss.timesteps_per_stage[s_start_stage][0].item()
            start_noise_ratio = start_noise_ratio / 1000.0
            init_factor = 2 ** (self.num_multiscale_stages - s_start_stage - 1)
            image_size = self.config.image_size // init_factor
            xT = torch.randn(quant.size(0), 3, image_size, image_size, device=quant.device, dtype=quant.dtype)
            noises = torch.randn_like(xT)
            xT = (1 - start_noise_ratio).view(-1,1,1,1) * xT + start_noise_ratio.view(-1,1,1,1) * noises
            ...
            for stage_idx in range(s_start_stage, self.num_multiscale_stages):
                # set the number of inference steps
                timestep_start = self.sit_loss.timesteps_per_stage[stage_idx][0].item()
                timestep_end = self.sit_loss.timesteps_per_stage[stage_idx][-1].item()
                timesteps = np.linspace(timestep_start, timestep_end, num_steps)
        ...
        return x_end_student_list, samples.to(torch.float32), s_start_stage, xT_start, diff, info
