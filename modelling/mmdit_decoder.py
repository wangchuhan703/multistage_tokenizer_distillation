...
class DismantledBlock(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs,
        ...
        assert attn_mode in self.ATTENTION_MODES
        if not rmsnorm:
            self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=disable_adaLN, eps=1e-6, dtype=dtype, device=device)
        else:
            self.norm1 = RMSNorm(hidden_size, elementwise_affine=disable_adaLN, eps=1e-6)
        ...
        if not pre_only:
            if not rmsnorm:
                self.norm2 = nn.LayerNorm(hidden_size, elementwise_affine=disable_adaLN, eps=1e-6, dtype=dtype, device=device)
            else:
                self.norm2 = RMSNorm(hidden_size, elementwise_affine=disable_adaLN, eps=1e-6)
...
class MMDiT(nn.Module):
    ...
    def __init__(
        ...
        # TODO: hand coded
        context_embedder_config = {"params": {"in_features": 32, "out_features": hidden_size}, "target": "torch.nn.Linear"}
        ...
        self.joint_blocks = nn.ModuleList(
            [
                JointBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, attn_mode=attn_mode, pre_only=i == depth - 1 if not self.output_register else False, rmsnorm=rmsnorm, scale_mod_only=scale_mod_only, swiglu=swiglu, qk_norm=qk_norm, dtype=dtype, device=device, disable_adaLN=self.output_register)
    ...
    def forward(
        ...
        if context_features is not None:
            context = self.context_embedder(context_features)
        else:
            context = None 
            
        if self.training and self.using_cfg and context is not None:
            b = x.shape[0]
            cfg_mask = (torch.rand((b,), device=context.device) > 0.1)[:, None, None]
            context = context * cfg_mask

        if self.output_register:
            context = self.forward_core_with_concat(x, c, context, detach)
            return context
        else:
...
def mmdit_d12(in_channels, input_size, patch_size, **kwargs):
    """
    MM-DiT model with 12 layers.
    """
    return MMDiT(
...
def mmdit_d24(in_channels, input_size, patch_size, **kwargs):
    """
    MM-DiT model with 12 layers.
    """
    return MMDiT(
