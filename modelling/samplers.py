...
def euler_maruyama_sampler(
    ...
    if cfg_scale != 1.0:
        # y_null = torch.tensor([num_classes] * y.size(0), device=y.device)
        y_null = model.context_null.repeat(y.size(0), 1, 1) # torch.zeros_like(y)
...
def multiscale_euler_sampler(
    ...
    with torch.no_grad():
        ...
        # for i, (t_cur, t_next) in enumerate(zip(t[:-1], t[1:])):
            x_cur = x_next
            if cfg_scale > 1.0 and timestep <= guidance_high and timestep >= guidance_low:
                model_input = torch.cat([x_cur] * 2, dim=0)
                y_cur = torch.cat([y, y_null], dim=0)
            else:
                model_input = x_cur
            ...
            if cfg_scale > 1. and timestep <= guidance_high and timestep >= guidance_low:
                d_cur_cond, d_cur_uncond = d_cur.chunk(2)
                d_cur = d_cur_uncond + cfg_scale * (d_cur_cond - d_cur_uncond)     
